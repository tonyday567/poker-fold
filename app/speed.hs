...
sortingChecks n t = do
  ...
  case t of
    -- 1.699
    "listsort" -> logTick "listsort" (fmap sort) (from cardsS7L cs)
    -- 0.912
...
handRankSpeeds n r = case r of
  ...
  "lookupHRUnsafe" ->
    logTick
      ...
      ( V.foldMap id $
          applyV
            (Cards2S . S.fromList . sort . S.toList . uncardsS)
            (card7sS n)
      )
  -- 0.359
  "lookupHR2" -> do
...
handRankComponents n r = case r of
  ...
  "rank" -> do
    ...
    logTick
      "rank for a flat"
      (applyFlat 7 (S.map (fromEnum . rank . toEnum . fromIntegral)))
      cs
